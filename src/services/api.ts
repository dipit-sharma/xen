...
export async function fetchRoute(destination: string, origin: string, sessionId: string): Promise<RouteResponse> {
    const res = await fetch(`${SERVER_BASE}/route`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ destination, origin, sessionId }),
    });
    if (!res.ok) {
